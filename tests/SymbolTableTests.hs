...
tests =
  describe "Symbol Table Tests" $ do
      ...
      it "function binding" $  "let f x = x" --> [
         ...
         "Symbol {name = f Just line 1 column 5, ty = b -> b, parent = Just (Symbol {name = f Just line 1 column 5, ty = a -> a, parent = Nothing, top = True}), top = True}"
       ]
      ...
      it "function application" $  "let f x y = x + y" --> [
         ...
         "Symbol {name = f Just line 1 column 5, ty = Num b => b -> b -> b, parent = Just (Symbol {name = f Just line 1 column 5, ty = Num a => a -> a -> a, parent = Nothing, top = True}), top = True}"
        ]