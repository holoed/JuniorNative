module Junior.TypeChecker.Infer where

import Data.Map (empty)
import Data.Set (fromList, insert, union)
import Junior.TypeChecker.Monads ( local, get, listen, run, throwError )
...
import Junior.TypeChecker.TypedAst ( TypedExp, tlit, tvar, tapp, tlam, tleT, tifThenElse, tmatch, tmkTuple, tvarPat, ttuplePat, tdefn, tmatchExp, tconPat, tlitPat )
import Junior.Core.Types ( TypeScheme(Identity), Type(..), Qual(..), clean, deleteTautology )
import Junior.Core.BuiltIns ( boolCon, intCon, doubleCon, strCon, charCon, tupleCon )
import Junior.TypeChecker.Environment ( Env, addScheme, findScheme, fromScheme, containsScheme )
import Junior.TypeChecker.Substitutions ( Substitutions, substituteQ )
import Junior.TypeChecker.InferMonad ( TypeM, newTyVar, getBaseType, getTypeForName, generalise, substituteQM, mkForAll )
import Junior.TypeChecker.Unification ( mgu )
