module Junior.Parser.Utils where

import Junior.Core.Types (Qual ((:=>)), Type(TyCon, TyVar, TyApp), Pred (IsIn), tyLam)
import Data.Set (Set, fromList)
import Junior.Parser.PAst (SynExp, SynExpF (Var, InfixApp, MkTuple))
import Junior.Utils.RecursionSchemes (cataRec)
import Junior.Utils.Annotations (Ann(Ann))
import Data.Char (isLower)
import Junior.Core.Operators (juxtaOp, classOp, lamOp)
