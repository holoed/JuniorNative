...
module Junior.Parser.SynExpToExp where
...
import Junior.Core.Operators ( juxtaOp, mulOp, divOp, plusOp, plusplusOp, exclexclOp, subOp, eqeqOp, gteqOp, lteqOp, andOp, orOp, gtOp, ltOp, consOp, ltStarGtOp, ltGtOp, noteqOp, gtEqGtOp, gtGtEqOp, ltDollarGtOp, powOp )
import Junior.Utils.RecursionSchemes ( cataRec )
import Junior.Parser.Location (Loc, zeroLoc)
...
fromExp = compressDefn . compressLets . compressLambdas . cataRec alg
    where alg (Ann (Just l) (Ast.Lit x)) = PAst.lit l x
          ...
          alg (Ann _ (Ast.App (In (Ann _ (PAst.InfixApp (" ", _, _) (In (Ann _ (PAst.Var "^"))) e1))) e2)) = PAst.infixApp zeroLoc powOp e1 e2
          ...
          alg (Ann _ (Ast.App (In (Ann _ (PAst.InfixApp (" ", _, _) (In (Ann _ (PAst.Var "!!"))) e1))) e2)) = PAst.infixApp zeroLoc exclexclOp e1 e2
