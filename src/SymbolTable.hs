...
extractSymbolsFromLet (In (Ann (_, qt) (Let n _ _))) = do
  ...
  return (n', Symbol { name = name s, ty = prettyQ qt, parent = Nothing, top = True })
extractSymbolsFromLet _ = []

extractSymbols :: TypedExp -> [(String, Symbol)]
extractSymbols (In (Ann (loc, qt) (VarPat s))) =
     [(s, Symbol { name = PStr(s, loc), ty = prettyQ qt, parent = Nothing, top = False })]
...
alg (Ann (loc, qt) (Lit v)) = do
    tell [Symbol { name = PStr(primToStr v, loc), ty = prettyQ qt, parent = Nothing, top = False }]
    return $ In (Ann (loc, qt) (Lit v))
alg (Ann (loc, qt) (Var n)) = do
    env <- ask
    tell [Symbol { name = PStr(n, loc), ty = prettyQ qt, parent = env!?n, top = False}]
    return $ In (Ann (loc, qt) (Var n))
alg (Ann (loc, qt) (VarPat n)) = do
    tell [Symbol { name = PStr(n, loc), ty = prettyQ qt, parent = Nothing, top = False}]
