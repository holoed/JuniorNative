...
import Ast (ExpF(VarPat, AppClosure, Var, Defn, Let))
import Data.Map ( insert, empty, member, Map, (!) )
import Debug.Trace (trace)
import Prelude hiding (lookup)
import Control.Monad ((>=>))

type OptimizeM = State (Map String String)
...
optimizeImp es = sequence (cataRec alg <$> es)
    where
        ...
        alg (Ann attr (Let n v b)) = do
            ...
            v' <- v
            case (n', v') of
             (In (Ann _ (VarPat k)) ,In (Ann _ (Var "=="))) -> do
                 state <- get
                 put (insert k "nativeEq" state)
                 b' <- b
        ...
        alg (Ann attr (Var n)) = do
            state <- get
            if member n state
            then return $ trace "replaced" $ In (Ann attr (Var (state!n)))
